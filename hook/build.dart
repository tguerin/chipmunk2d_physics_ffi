...
void main(List<String> args) async {
  await build(args, (input, output) async {
    ...
    final iosTargetSdk = targetOS == OS.iOS ? input.config.code.iOS.targetSdk : null;
...
Future<String?> downloadPrebuiltLibrary({
  ...
  final archName = _mapArchName(architecture, targetOS, iosTargetSdk);
...
String _mapArchName(Architecture arch, OS os, IOSSdk? iosTargetSdk) {
  ...
  if (os == OS.iOS) {
    ...
    if (iosTargetSdk == null) {
      throw Exception('iosTargetSdk is required for iOS builds');
    }
    final archName = arch.name; // e.g., "arm64
    return '$archName-${iosTargetSdk.type}';
  }

...
///
/// Uses dart:js_interop to call pure WASM module directly.
/// The WASM module is automatically loaded from the package's assets.
library;
...
const _defaultWasmPath = 'assets/packages/chipmunk2d_physics_ffi/assets/web/chipmunk2d_physics_ffi.wasm';

/// Internal initialization function for direct WASM loading.
Future<void> _doInitialize() async {
  ...
  try {
    // Try instantiateStreaming first (faster, but requires correct MIME type)
    // If that fails due to MIME type error, fall back to loading bytes
    JSObject result;
    final imports = JSObject();
    final env = JSObject();
    imports.setProperty('env'.toJS, env);
    try {
      final responsePromise = web.window.fetch(_defaultWasmPath.toJS);
      final wasmPromise = web.WebAssembly.instantiateStreaming(responsePromise, imports);
      result = await wasmPromise.toDart;
    } catch (e) {
      // Fallback: Load as ArrayBuffer and use instantiate
      // This works even if the server doesn't set the correct MIME type (application/wasm)
      final responsePromise = web.window.fetch(_defaultWasmPath.toJS);
      ...
      if (!webResponse.ok) {
        throw StateError(
          'Failed to fetch WASM file from "$_defaultWasmPath". '
          'HTTP status: ${webResponse.status}.',
        );
      }

      final arrayBufferPromise = webResponse.arrayBuffer();
      final arrayBuffer = await arrayBufferPromise.toDart;

      final wasmPromise = web.WebAssembly.instantiate(arrayBuffer, imports);
      result = await wasmPromise.toDart;
    }

    // Extract the instance and exports
    final instance = result.getProperty('instance'.toJS) as JSObject?;
    if (instance == null) {
      throw StateError('WASM instantiation returned null instance');
    }

    final exports = instance.getProperty('exports'.toJS) as JSObject?;
    if (exports == null) {
      throw StateError('WASM instance has no exports');
    }

    // Store exports and memory directly
    _wasmExports = exports;
    _wasmMemory = exports.getProperty('memory'.toJS) as JSObject?;

    if (_wasmMemory == null) {
      throw StateError('WASM instance does not export memory');
    }

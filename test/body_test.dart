...
void main() {
  group('Body', () {
    ...
    test('sets and gets position', () {
      final body = Body.dynamic(1, 1);
      const position = Vector(10, 20);
      body.position = position;
      expect(body.position.x, closeTo(10, 0.001));
      expect(body.position.y, closeTo(20, 0.001));
      body.dispose();
    });

    test('fast position getters', () {
      final body = Body.dynamic(1, 1)..position = const Vector(10, 20);
      expect(body.positionX, closeTo(10, 0.001));
      expect(body.positionY, closeTo(20, 0.001));
      body.dispose();
    });

    test('sets and gets velocity', () {
      final body = Body.dynamic(1, 1);
      const velocity = Vector(5, -5);
      body.velocity = velocity;
      expect(body.velocity.x, closeTo(5, 0.001));
      expect(body.velocity.y, closeTo(-5, 0.001));
      body.dispose();
    });

    test('fast velocity getters', () {
      final body = Body.dynamic(1, 1)..velocity = const Vector(5, -5);
      expect(body.velocityX, closeTo(5, 0.001));
      expect(body.velocityY, closeTo(-5, 0.001));
      body.dispose();
    });

    test('sets and gets angle', () {
      final body = Body.dynamic(1, 1)..angle = 1.57; // ~90 degrees
      expect(body.angle, closeTo(1.57, 0.001));
      body.dispose();
    });

    test('sets and gets mass', () {
      final body = Body.dynamic(2, 1);
      expect(body.mass, closeTo(2, 0.001));
      body.mass = 3;
      expect(body.mass, closeTo(3, 0.001));
      body.dispose();
    });

    test('sets and gets moment', () {
      final body = Body.dynamic(1, 2);
      expect(body.moment, closeTo(2, 0.001));
      body.moment = 3;
      expect(body.moment, closeTo(3, 0.001));
      body.dispose();
    });

    test('sets and gets angular velocity', () {
      final body = Body.dynamic(1, 1)..angularVelocity = 1.5;
    ...
    test('sets and gets center of gravity', () {
      ...
      final body = Body.dynamic(1, 1)..centerOfGravity = cog;
    ...
    test('sets and gets force', () {
      const force = Vector(10, 20);
      final body = Body.dynamic(1, 1)..force = force;
      final retrieved = body.force;
      expect(retrieved.x, closeTo(10, 0.001));
      expect(retrieved.y, closeTo(20, 0.001));
      body.dispose();
    });

    test('sets and gets torque', () {
      final body = Body.dynamic(1, 1)..torque = 5;
      expect(body.torque, closeTo(5, 0.001));
      body.dispose();
    });

    test('gets rotation vector', () {
      final body = Body.dynamic(1, 1)..angle = 0;
      final rotation = body.rotation;
      expect(rotation.x, closeTo(1, 0.001));
      expect(rotation.y, closeTo(0, 0.001));
      body.dispose();
    });

    test('sets body type', () {
      final body = Body.dynamic(1, 1)..type = BodyType.kinematic;
      expect(body.type, BodyType.kinematic);
      body.dispose();
    });
    ...
    test('activate wakes up body', () {
      final space = Space()..sleepTimeThreshold = 0.5; // Enable sleeping
      final body = Body.dynamic(1, 1);
      space.addBody(body);
      body.sleep();
    ...
    test('localToWorld conversion', () {
      const localPoint = Vector(1, 0);
      final body = Body.dynamic(1, 1)
        ..position = const Vector(10, 20)
        ..angle = 0;
      final worldPoint = body.localToWorld(localPoint);
      expect(worldPoint.x, closeTo(11, 0.001));
      expect(worldPoint.y, closeTo(20, 0.001));
      body.dispose();
    });

    test('worldToLocal conversion', () {
      const worldPoint = Vector(11, 20);
      final body = Body.dynamic(1, 1)
        ..position = const Vector(10, 20)
        ..angle = 0;
      final localPoint = body.worldToLocal(worldPoint);
      expect(localPoint.x, closeTo(1, 0.001));
      expect(localPoint.y, closeTo(0, 0.001));
      body.dispose();
    });

    test('applyForceAtWorldPoint', () {
      const force = Vector(10, 0);
    ...
    test('getVelocityAtWorldPoint', () {
      const point = Vector.zero;
      final body = Body.dynamic(1, 1)..velocity = const Vector(5, 0);
      final velocity = body.getVelocityAtWorldPoint(point);
      expect(velocity.x, closeTo(5, 0.001));
      body.dispose();
    });
    ...
    test('kineticEnergy calculation', () {
      final body = Body.dynamic(1, 1)
        ..velocity = const Vector(10, 0)
        ..angularVelocity = 1;
      final ke = body.kineticEnergy;
      expect(ke, greaterThan(0));
      body.dispose();
    });

    test('disposed flag is set after disposal', () {
      final body = Body.dynamic(1, 1);
      expect(body.disposed, false);
      body.dispose();
      expect(body.disposed, true);
    });

    test('fromNative creates body from pointer', () {
      final original = Body.dynamic(1, 1);

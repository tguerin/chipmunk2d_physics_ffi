...
void main() {
  group('Space', () {
    ...
    test('adds and removes shape', () {
      ...
      // Remove body from space before disposing
      space.removeBody(body);
    ...
    test('adds and removes constraint', () {
      final space = Space();
      final bodyA = Body.dynamic(1, 1);
      final bodyB = Body.dynamic(1, 1);
      ...
      // Remove bodies from space before disposing
      space
        ..removeBody(bodyA)
        ..removeBody(bodyB);
      // Dispose manually since we removed from space
    ...
    test('steps simulation', () {
      final space = Space();
      final body = Body.dynamic(1, 1)..position = const Vector(0, 100);
      final shape = CircleShape(body, 10);
      space
        ..addBody(body)
        ..addShape(shape);
      expect(() => space.step(1 / 60.0), returnsNormally);
    ...
    test('disposed flag is set after disposal', () {
      final space = Space();
      expect(space.disposed, false);
      space.dispose();
      expect(space.disposed, true);
    });

  ...
  group('Shape', () {
    ...
    group('CircleShape', () {
      test('creates circle shape', () {
        final shape = CircleShape(body, 10);
        expect(shape, isNotNull);
        expect(shape.radius, closeTo(10, 0.001));
        shape.dispose();
      });

      test('creates circle shape with offset', () {
        const offset = Vector(5, 5);
        final shape = CircleShape(body, 10, offset: offset);
        expect(shape.offset.x, closeTo(5, 0.001));
        expect(shape.offset.y, closeTo(5, 0.001));
        shape.dispose();
      });

      test('gets radius', () {
        final shape = CircleShape(body, 15);
        expect(shape.radius, closeTo(15, 0.001));
        shape.dispose();
      });

      test('gets offset', () {
        const offset = Vector(2, 3);
        final shape = CircleShape(body, 10, offset: offset);
        final retrieved = shape.offset;
        expect(retrieved.x, closeTo(2, 0.001));
        expect(retrieved.y, closeTo(3, 0.001));
        shape.dispose();
      });
    });
    ...
    group('BoxShape', () {
      test('creates box shape', () {
        final shape = BoxShape(body, 20, 30);
        expect(shape, isNotNull);
        shape.dispose();
      });

      test('creates box shape with radius', () {
        final shape = BoxShape(body, 20, 30, radius: 2);
        expect(shape, isNotNull);
        shape.dispose();
      });
    });
    ...
    group('SegmentShape', () {
      test('creates segment shape', () {
        ...
        expect(shape, isNotNull);
        expect(shape.radius, closeTo(2, 0.001));
        shape.dispose();
      });
      ...
      test('gets endpoints', () {
        ...
        final shape = SegmentShape(body, a, b, 2);
        expect(shape.endpointA.x, closeTo(0, 0.001));
        expect(shape.endpointB.x, closeTo(100, 0.001));
        shape.dispose();
      });
      ...
      test('gets normal', () {
        ...
        expect(normal.length, closeTo(1, 0.001));
        shape.dispose();
      });
    ...
    group('PolyShape', () {
      ...
      test('creates poly shape with radius', () {
        ...
        final shape = PolyShape(body, vertices, radius: 1);
        expect(shape.radius, closeTo(1, 0.001));
        shape.dispose();
      });
    ...
    group('Shape properties', () {
      test('sets and gets friction', () {
        final shape = CircleShape(body, 10)..friction = 0.7;
        expect(shape.friction, closeTo(0.7, 0.001));
        shape.dispose();
      });

      test('sets and gets elasticity', () {
        final shape = CircleShape(body, 10)..elasticity = 0.5;
        expect(shape.elasticity, closeTo(0.5, 0.001));
        shape.dispose();
      });

      test('sets and gets mass', () {
        final shape = CircleShape(body, 10)..mass = 2;
        expect(shape.mass, closeTo(2, 0.001));
        shape.dispose();
      });

      test('sets and gets density', () {
        final shape = CircleShape(body, 10)..density = 1.5;
      ...
      test('gets moment', () {
        ...
        // Moment can be 0 if shape isn't in a space or mass properties aren't set
        expect(moment, greaterThanOrEqualTo(0));
        shape.dispose();
      });
      ...
      test('sets and gets sensor flag', () {
        final shape = CircleShape(body, 10)..sensor = true;
      ...
      test('sets and gets surface velocity', () {
        final shape = CircleShape(body, 10);
        const surfaceVel = Vector(5, 0);
        shape.surfaceVelocity = surfaceVel;
        final retrieved = shape.surfaceVelocity;
        expect(retrieved.x, closeTo(5, 0.001));
        shape.dispose();
      });

      test('sets and gets collision type', () {
        final shape = CircleShape(body, 10)..collisionType = 1;
    ...
    test('disposed flag is set after disposal', () {
      final shape = CircleShape(body, 10);
      expect(shape.disposed, false);
      shape.dispose();
      expect(shape.disposed, true);
    });
  });

...
void main() {
  group('Integration Tests', () {
    test('simple physics simulation', () {
      ...
      final ball = Body.dynamic(1, 1)..position = const Vector(0, 100);
      final ballShape = CircleShape(ball, 10)
        ..friction = 0.7
        ..elasticity = 0.5;
    ...
    test('multiple bodies interaction', () {
      ...
      for (var i = 0; i < 5; i++) {
        final body = Body.dynamic(1, 1)..position = Vector(i * 20, 100);
        final shape = CircleShape(body, 5);
    ...
    test('collision detection', () {
      final space = Space();
      final bodyA = Body.dynamic(1, 1)..position = const Vector(0, 50);
      final bodyB = Body.dynamic(1, 1)..position = const Vector(5, 50);
      final shapeA = CircleShape(bodyA, 10);
      final shapeB = CircleShape(bodyB, 10);
      ...
      expect(bodyA.position.x, isNot(closeTo(0, 0.1)));
      expect(bodyB.position.x, isNot(closeTo(5, 0.1)));
    ...
    test('joint constraint', () {
      final space = Space();
      final bodyA = Body.dynamic(1, 1)..position = const Vector(0, 50);
      final bodyB = Body.dynamic(1, 1)..position = const Vector(0, 30);
      final shapeA = CircleShape(bodyA, 5);
      final shapeB = CircleShape(bodyB, 5);
      ...
      expect(distance, closeTo(20, 5)); // Allow some tolerance
    ...
    test('sleeping bodies', () {
      final space = Space()..gravity = const Vector(0, -100);
      final body = Body.dynamic(1, 1)..position = Vector.zero;
    ...
    test('shape queries', () {
      final space = Space();
      final bodyA = Body.dynamic(1, 1)..position = Vector.zero;
      final shapeA = CircleShape(bodyA, 10);
      final bodyB = Body.dynamic(1, 1)..position = const Vector(5, 0);
      final shapeB = CircleShape(bodyB, 10);
    ...
    test('all joint types work together', () {
      ...
      // Create bodies
      for (var i = 0; i < 5; i++) {
        final body = Body.dynamic(1, 1)..position = Vector(i * 20, 50);
        bodies.add(body);
        space.addBody(body);
      }
      ...
      // Cleanup - remove constraints from space before disposing
    ...
    test('memory management', () {
      ...
      // Cleanup - remove constraints and shapes before disposing space
      for (final constraint in constraints) {
        space.removeConstraint(constraint);
        constraint.dispose();
      }

...
void main() {
  group('Vector', () {
    test('creates vector with x and y components', () {
      ...
      expect(vector.x, 10.0);
      expect(vector.y, 20.0);
    });
    ...
    test('vector subtraction', () {
      const v1 = Vector(5.0, 7.0);
      const v2 = Vector(2.0, 3.0);
      final result = v1 - v2;
      expect(result.x, 3.0);
      expect(result.y, 4.0);
    });
    ...
    test('dot product', () {
      ...
      expect(v1.dot(v2), 11.0); // 1*3 + 2*4 = 11
    });

    test('dot product with zero vector', () {
      const v1 = Vector(1.0, 2.0);
      expect(v1.dot(Vector.zero), 0.0);
    });

    test('cross product', () {
      const v1 = Vector(1.0, 0.0);
      const v2 = Vector(0.0, 1.0);
      expect(v1.cross(v2), 1.0); // Right-hand rule
    });
    ...
    test('length calculation', () {
      ...
      expect(v.length, closeTo(5.0, 0.001)); // 3-4-5 triangle
    });

    test('length of zero vector', () {
      expect(Vector.zero.length, 0.0);
    });

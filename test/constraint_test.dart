...
void main() {
  group('Constraint', () {
    ...
    setUp(() {
      bodyA = Body.dynamic(1.0, 1.0);
      bodyB = Body.dynamic(1.0, 1.0);
    });
    ...
    group('PinJoint', () {
      ...
      test('sets and gets anchors', () {
        ...
        expect(joint.anchorA.x, closeTo(1.0, 0.001));
        expect(joint.anchorB.y, closeTo(1.0, 0.001));
        joint.dispose();
      });
      ...
      test('sets and gets distance', () {
        ...
        expect(joint.distance, closeTo(10.0, 0.001));
        joint.dispose();
      });
    });
    ...
    group('SlideJoint', () {
      ...
      test('sets and gets min/max', () {
        ...
        expect(joint.min, closeTo(5.0, 0.001));
        expect(joint.max, closeTo(10.0, 0.001));
        joint
          ..min = 3.0
          ..max = 15.0;
        expect(joint.min, closeTo(3.0, 0.001));
    ...
    group('GrooveJoint', () {
      ...
      test('sets and gets groove endpoints', () {
        ...
        expect(joint.grooveA.x, closeTo(1.0, 0.001));
    ...
    group('DampedSpring', () {
      ...
      test('sets and gets stiffness', () {
        ...
        expect(joint.stiffness, closeTo(2.0, 0.001));
        joint.dispose();
      });
    ...
    group('DampedRotarySpring', () {
      ...
      test('sets and gets rest angle', () {
        ...
        expect(joint.restAngle, closeTo(1.0, 0.001));
        joint.dispose();
      });
    ...
    group('RotaryLimitJoint', () {
      test('creates rotary limit joint', () {
        final joint = RotaryLimitJoint(bodyA, bodyB, -1.0, 1.0);
        expect(joint, isNotNull);
        joint.dispose();
      });
      ...
      test('sets and gets min/max', () {
        final joint = RotaryLimitJoint(bodyA, bodyB, -1.0, 1.0);
        expect(joint.min, closeTo(-1.0, 0.001));
        expect(joint.max, closeTo(1.0, 0.001));
        joint
          ..min = -2.0
          ..max = 2.0;
        expect(joint.min, closeTo(-2.0, 0.001));
        expect(joint.max, closeTo(2.0, 0.001));
        joint.dispose();
      });
    });
    ...
    group('RatchetJoint', () {
      test('creates ratchet joint', () {
        final joint = RatchetJoint(bodyA, bodyB, 0.0, 1.0);
        expect(joint, isNotNull);
        joint.dispose();
      });

      test('sets and gets phase', () {
        final joint = RatchetJoint(bodyA, bodyB, 0.0, 1.0)..phase = 0.5;
        expect(joint.phase, closeTo(0.5, 0.001));
        joint.dispose();
      });

      test('sets and gets ratchet', () {
        final joint = RatchetJoint(bodyA, bodyB, 0.0, 1.0)..ratchet = 2.0;
        expect(joint.ratchet, closeTo(2.0, 0.001));
        joint.dispose();
      });
    });
    ...
    group('GearJoint', () {
      test('creates gear joint', () {
        final joint = GearJoint(bodyA, bodyB, 0.0, 1.0);
        expect(joint, isNotNull);
        joint.dispose();
      });

      test('sets and gets phase', () {
        final joint = GearJoint(bodyA, bodyB, 0.0, 1.0)..phase = 0.5;
        expect(joint.phase, closeTo(0.5, 0.001));
        joint.dispose();
      });

      test('sets and gets ratio', () {
        final joint = GearJoint(bodyA, bodyB, 0.0, 1.0)..ratio = 2.0;
        expect(joint.ratio, closeTo(2.0, 0.001));
        joint.dispose();
      });
    });
    ...
    group('SimpleMotor', () {
      test('creates simple motor', () {
        final joint = SimpleMotor(bodyA, bodyB, 1.0);
        expect(joint, isNotNull);
        joint.dispose();
      });

      test('sets and gets rate', () {
        final joint = SimpleMotor(bodyA, bodyB, 1.0)..rate = 2.0;
        expect(joint.rate, closeTo(2.0, 0.001));
        joint.dispose();
      });
    });
    ...
    group('Constraint properties', () {
      ...
      test('throws error after disposal', () {
        ...
        expect(() => joint.maxForce, throwsStateError);
        expect(() => joint.bodyA, throwsStateError);
      });
    });
  });
}
